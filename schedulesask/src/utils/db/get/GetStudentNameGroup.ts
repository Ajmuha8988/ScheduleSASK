...
interface StudentNameGroup {
    NameGroup: string;
}
// Кастомный хук для получения данных
export const GetStudentNamegroup = () => {
    const [dataGroupName, setGroupName] = useState<StudentNameGroup[]>([]); // Данные о ролях
    const [loading, setLoading] = useState(true); // Статус загрузки

    useEffect(() => {
        fetch(API_URL + '/student/GroupInStudent', { credentials: 'include' })
