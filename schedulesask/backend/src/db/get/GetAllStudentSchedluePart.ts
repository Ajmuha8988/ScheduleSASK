...
export const getStudentSchedulePart = async (req: Request, res: Response) => {
    try {
        ...
        const result = await request.input('id_nameGroup', sql.NVarChar, decodedToken.id)
            .query(`With StudentSchedulePart as (
	                    Select NameLesson, NameGroup, NameRoom, 
						TimeDate, NumberLessons,
		                ...
		                Select *
		                From StudentSchedulePart
						Where ID_Group = (Select ID_Groups
						From Member_groups
						Where ID_Students = @id_nameGroup)`);
